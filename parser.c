...
char *lower_case_word(char *word);
...
char *next_word(FILE *infile, regex_t *word_regex) {
    ...
    index = (index + 1) % count;
    word = lower_case_word(word);
    return word;
}

char *lower_case_word(char *word) {
    int i = 0; // Initialize an index for word.
    while (word[i]) {
        word[i] = tolower(word[i]); // Change all the upper cases into lower cases.
        i += 1;
    }
    return word;
}
